...
func (gts *GroupTokenBucketState) balanceSlotTokens(
	...
	for _, slot := range gts.tokenSlots {
		if gts.clientConsumptionTokensSum == 0 || len(gts.tokenSlots) == 1 {
			...
			log.Info("resource group",
				zap.Uint64("slot.settings.BurstLimit", uint64(slot.settings.BurstLimit)),
				zap.Uint64("slot.settings.FillRate", slot.settings.FillRate),
				zap.Float64("evenRatio", evenRatio),
				zap.Any("gts.tokenSlots", gts.tokenSlots),
				zap.Uint64("settings.GetFillRate()", settings.GetFillRate()))
		}
	}
	if requiredToken != 0 {
		// Only slots that require a positive number will be considered alive.
